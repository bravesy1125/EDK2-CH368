...
VOID HelloUEFI(VOID)
{
  ...
  while(key.ScanCode!=0x17)	//ESC
  {
    ...
    if(key.UnicodeChar == 0x31)   
      SetLed(MyIoBaseAddr,LED1);
    if(key.UnicodeChar == 0x32)   
      SetLed(MyIoBaseAddr,LED2);
    if(key.UnicodeChar == 0x33)   
      SetLed(MyIoBaseAddr,LED3);
    if(key.UnicodeChar == 0x34)   
      SetLed(MyIoBaseAddr,LED4);
...
//
VOID SetLed(UINT16 IoBaseAddress,UINT8 LedNum)
{
  static UINT8 regValue=0xFF;
  switch (LedNum)
  {
    case LED1:regValue ^= 0x01;break;
    case LED2:regValue ^= 0x02;break;
    case LED3:regValue ^= 0x04;break;
    case LED4:regValue ^= 0x08;break;
  }
  IoWrite8(MyIoBaseAddr,regValue);    
} 